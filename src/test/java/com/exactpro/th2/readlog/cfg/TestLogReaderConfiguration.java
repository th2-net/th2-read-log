...
class TestLogReaderConfiguration {
    ...
    void deserializes() throws IOException {
        ...
        try(var input = TestLogReaderConfiguration.class.getClassLoader().getResourceAsStream("test_cfg.json")) {
            ...
            assertEquals(Set.of(Direction.FIRST, Direction.SECOND), Objects.requireNonNull(cfg.getAliases().get("A").getDirectionToPattern().keySet()));
            assertEquals(Set.of(Direction.FIRST), Objects.requireNonNull(cfg.getAliases().get("B").getDirectionToPattern().keySet()));
